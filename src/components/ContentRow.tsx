...
export const ContentRow: React.FC<ContentRowProps> = ({
  ...
  const scroll = (direction: 'left' | 'right') => {
    if (rowRef.current) {
      ...
      let factor = direction === 'left' ? -1 : 1;
      if (isRtl) {
        factor = -factor;
      }
      const scrollAmount = factor * 600;
  ...
  return (
    <section className="relative py-4 group/row">
      ...
      <div className="relative">
        ...
        <button
          onClick={() => scroll('left')}
          className="absolute -start-2 sm:start-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-black/70 hover:bg-violet-600 border border-white/10 hover:border-violet-400 text-white flex items-center justify-center backdrop-blur-md opacity-0 group-hover/row:opacity-100 transition-all duration-200 shadow-xl cursor-pointer"
        ...
        <button
          ...
          className="absolute -end-2 sm:end-2 top-1/2 -translate-y-1/2 z-20 w-10 h-10 rounded-full bg-black/70 hover:bg-violet-600 border border-white/10 hover:border-violet-400 text-white flex items-center justify-center backdrop-blur-md opacity-0 group-hover/row:opacity-100 transition-all duration-200 shadow-xl cursor-pointer"
